...
function btnStyle($app, settings) {
    ...
    if ( settings.buttons.border_width > 3) { settings.buttons.border_width = 0; }
...
function arrowStyle($app, settings) {
    ...
    $app.find('.WS-lb-arrow--left i').removeClass()
        .addClass(`${settings.arrows.icon_style}-left`);
    $app.find('.WS-lb-arrow--right i').removeClass()
        .addClass(`${settings.arrows.icon_style}-right`);
    ...
    if ( settings.arrows.icon_size > $arrows.width() ) { settings.arrows.icon_size = $arrows.width(); }
   
    $arrows.css({ 'font-size' : `${settings.arrows.icon_size}px`,
                  'opacity'   : settings.arrows.opacity })
        .addClass(`${settings.arrows.hover_effect}-hover`);
...
function imageContainer($app, settings) {
    ...
    $app.find('.WS-lightbox--subcontainer')
        .css('background', settings.image.bg)
    .find('img')
        .css({
