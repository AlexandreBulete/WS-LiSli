...
function transitionFade($app, $img_n1, settings) {
    ...
    $app.find('.WS-lightbox--container img').fadeOut(settings.slide_velocity).attr('data-item-order', $getCurrentOrder);
    setTimeout(function () {
...
function currentBg($app, settings, $getSrc) {
    // if ( settings.background.filter === 'image' ) {
    $app.find('.WS-lightbox--bg').fadeOut(settings.slide_velocity);
    setTimeout(function () {
        $app.find('.WS-lightbox--bg').css('background-image', `url(${$getSrc})`);
    }, settings.slide_velocity);
    $app.find('.WS-lightbox--bg').fadeIn(settings.slide_velocity);
    // }
}