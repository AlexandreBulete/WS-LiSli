...
function btnStyle($app, options) {
    ...
    if (options.buttons.border_width > 3) {
        options.buttons.border_width = 0;
    }
...
function arrowStyle($app, options) {
    ...
    switch (options.arrows.style) {
        case 'circle':
            ...
            break;
        case 'svelt':
            $arrows.css('height', '150px').parent().css('bottom', `calc(50% + 75px)`);
            break;
        default:
            break;
    }
    $app.find('.WS-lb-arrow--left i').removeClass().addClass(`${options.arrows.icon_style}-left`);
    $app.find('.WS-lb-arrow--right i').removeClass().addClass(`${options.arrows.icon_style}-right`);
    ...
    if (options.arrows.icon_size > $arrows.width()) {
        options.arrows.icon_size = $arrows.width();
    }

    $arrows.css({ 'font-size': `${options.arrows.icon_size}px`,
        'opacity': options.arrows.opacity }).addClass(`${options.arrows.hover_effect}-hover`);
...
function imageContainer($app, options) {
    ...
    $app.find('.WS-lightbox--subcontainer').css('background', options.image.bg).find('img').css({
